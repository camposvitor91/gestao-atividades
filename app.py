...
@contextmanager
def get_conn():
    """Context manager que garante commit, rollback e fechamento automático."""
    conn = conectar()
    try:
        yield conn
        conn.commit()
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS atividades (
                ...
                origem               TEXT,
                criado_por           TEXT,
                aprovacao_gestor     TEXT,
                aprovacao_conclusao  TEXT,
...
def registrar_historico(atividade_id: int, acao: str, campo: str = "",
    ...
    with get_conn() as conn:
        conn.execute(
            """
            INSERT INTO historico (atividade_id, data_hora, acao, campo, valor_anterior, valor_novo, feito_por)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
...
def listar_historico(atividade_id: int) -> pd.DataFrame:
    ...
    df = pd.read_sql_query(
        "SELECT * FROM historico WHERE atividade_id = ? ORDER BY id DESC",
        conn, params=(atividade_id,)
    )
    conn.close()
    return df
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            INSERT INTO atividades (
                tipo, macro_acao, micro_acao, criticidade, frequencia,
                data_referencia, tempo, prazo, status, responsavel,
                origem, criado_por, aprovacao_gestor, aprovacao_conclusao,
...
def importar_excel(arquivo) -> int:
    ...
    for aba in abas:
        ...
        for _, row in df.iterrows():
            ...
            tempo_valor = obter_valor_linha(row, "TEMPO", default=0)
            try:
                tempo_valor = float(tempo_valor) if pd.notna(tempo_valor) else 0.0
            except Exception:
                tempo_valor = 0.0

            percentual_valor = obter_valor_linha(row, "% CONCLUSÃO", "% CONCLUSAO", default=0)
            try:
                percentual_valor = float(percentual_valor) if pd.notna(percentual_valor) else 0.0
            except Exception:
            ...
            if pd.notna(prazo_valor) and str(prazo_valor).strip() != "":
                try:
                    prazo_valor = pd.to_datetime(prazo_valor).strftime("%d/%m/%Y")
                except Exception:
                    prazo_valor = str(prazo_valor).strip()
            else:
                prazo_valor = ""

            responsavel_valor = obter_valor_linha(
                row,
                "RESPONSÁVEL 1", "RESPONSAVEL 1",
...
def gerar_recorrencia(id_atividade: int) -> bool:
    """
    Verifica se a atividade é recorrente e, se for,
    cria automaticamente a próxima ocorrência com o próximo prazo.
    Retorna True se uma nova ocorrência foi criada.
    """
    df = listar()
    ...
    # Usa o prazo atual como data de referência para o próximo ciclo
    prazo_atual_str = str(r["prazo"]).strip()
    try:
        from datetime import datetime as _dt
        prazo_atual_dt = _dt.strptime(prazo_atual_str, "%d/%m/%Y").date()
    except Exception:
        return False
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            (
                ...
                str(r["criticidade"]),
                frequencia,
                prazo_atual_str,          # data_referencia = prazo do ciclo anterior
                float(r["tempo"]),
                proximo_prazo_str,
                "NÃO INICIADA",
                str(r["responsavel"]),
    ...
    registrar_historico(
        ...
        "Recorrência automática",
        "origem",
        f"gerada a partir da atividade #{id_atividade}",
        proximo_prazo_str,
        "Sistema",
    )
    listar.clear()
...
def reprovar_inicio(id_atividade: int) -> None:
    with get_conn() as conn:
        conn.execute(
            """
            UPDATE atividades
            SET aprovacao_gestor = 'REPROVADA',
...
def aprovar_conclusao(id_atividade: int) -> None:
    with get_conn() as conn:
        conn.execute(
            """
            UPDATE atividades
            SET aprovacao_conclusao = 'APROVADA',
                status = 'CONCLUÍDA',
...
def reprovar_conclusao(id_atividade: int) -> None:
    with get_conn() as conn:
        conn.execute(
            """
            UPDATE atividades
            SET aprovacao_conclusao = 'REPROVADA',
                status = 'EM ANDAMENTO'
            WHERE id = ?
            """,
            (id_atividade,),
...
def editar_atividade(id_atividade: int, dados: dict) -> None:
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            """,
            (
                dados["tipo"], dados["macro_acao"], dados["micro_acao"],
                dados["criticidade"], dados["frequencia"], dados["tempo"],
                dados["prazo"], dados["responsavel"], id_atividade,
            ),
        )
...
def gerar_excel(df: pd.DataFrame) -> bytes:
    ...
    import io
    colunas = [
        "id", "responsavel", "tipo", "macro_acao", "micro_acao",
        "criticidade", "frequencia", "tempo", "prazo", "status",
        "percentual_conclusao", "score", "horas_em_aberto",
    ]
    colunas_existentes = [c for c in colunas if c in df.columns]
    df_export = df[colunas_existentes].copy()
    df_export.columns = [c.replace("_", " ").upper() for c in df_export.columns]

    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        df_export.to_excel(writer, index=False, sheet_name="Atividades")
...
def atualizar_status(id_atividade: int, novo_status: str, percentual: float) -> None:
    ...
    criado_por    = str(row_atual["criado_por"].values[0])           if not row_atual.empty else ""
    with get_conn() as conn:
        conn.execute(
            """
            UPDATE atividades
    ...
    registrar_historico(id_atividade, "Atualização de status",
                        "status", status_ant, novo_status)
    if str(int(percentual)) != str(int(float(perc_ant or 0))):
        registrar_historico(id_atividade, "Atualização de %",
                            "% conclusão", perc_ant, str(int(percentual)))
    listar.clear()
    # Recorrência gerada apenas via aprovação de conclusão (aprovar_conclusao)
...
def converter_prazo_misto(serie: pd.Series) -> tuple[pd.Series, pd.Series]:
    ...
    mascara_restante = prazo_dt.isna() & prazo_texto.ne("")
    if mascara_restante.any():
        prazo_dt.loc[mascara_restante] = pd.to_datetime(
            prazo_texto.loc[mascara_restante], errors="coerce"
        )

    prazo_final = prazo_texto.copy()
    mascara_ok  = prazo_dt.notna()
    prazo_final.loc[mascara_ok] = prazo_dt.loc[mascara_ok].dt.strftime("%d/%m/%Y")
...
def preparar_dashboard(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Vetorizado com pd.cut — sem apply linha a linha
    df["peso_prazo"] = pd.cut(
        dias_para_prazo,
    ...
    df["score"] = (
        df["peso_criticidade"] * 3
        + df["peso_frequencia"] * 2
        + df["peso_status"]    * 2
        + df["peso_prazo"]     * 3
        + df["peso_tempo"]
    ).round(2)
...
def aplicar_estilo_carga(df: pd.DataFrame):
    """Estilo dedicado para a tabela Carga por Responsável com centralização real."""
    cols = list(df.columns)

    # Formatos por coluna
    formatos = {
        "QUANTIDADE DE ATIVIDADES": "{:,.0f}",
        "HORAS EM ABERTO":          "{:,.2f}",
    }
    styler = df.style.format(
        {k: v for k, v in formatos.items() if k in cols},
        decimal=",", thousands=".",
    )

    # set_table_styles gera CSS de classe — mais confiável que set_properties no Streamlit
    table_styles = []
    for col in ["QUANTIDADE DE ATIVIDADES", "HORAS EM ABERTO"]:
        if col in cols:
            i = cols.index(col)
            table_styles += [
                {"selector": f"td.col{i}",               "props": [("text-align", "center"), ("font-weight", "600")]},
                {"selector": f"th.col_heading.col{i}",   "props": [("text-align", "center")]},
            ]
...
def aplicar_estilo_tabela(df: pd.DataFrame, coluna_atraso: str | None = None):
    def destacar_atraso(row):
        if coluna_atraso and coluna_atraso in row.index and bool(row[coluna_atraso]):
            return ["background-color: #5c1a1a"] * len(row)
        return [""] * len(row)

    styler = df.style

    if coluna_atraso and coluna_atraso in df.columns:
        styler = styler.apply(destacar_atraso, axis=1)

    cols_numericas = df.select_dtypes(include=["number"]).columns.tolist()

    if cols_numericas:
        styler = styler.set_properties(subset=cols_numericas, **{"text-align": "center"})
        formatos = {}
        for col in cols_numericas:
            if col == "QUANTIDADE DE ATIVIDADES":
                formatos[col] = "{:,.0f}"
            elif col == "%_CONCLUSÃO":
                formatos[col] = "{:,.0f}%"
            else:
                formatos[col] = "{:,.2f}"
        styler = styler.format(formatos, decimal=",", thousands=".")
...
def fazer_backup() -> None:
    """Copia o banco para backups/ uma vez por dia."""
    import shutil
    from datetime import datetime as _dt
    import os
    ...
    nome_backup = f"{pasta}/gestao_atividades_{_dt.today().strftime('%Y%m%d')}.db"

    # Só faz backup se ainda não existir o arquivo de hoje
    if not os.path.exists(nome_backup):
        ...
        for arquivo_antigo in arquivos[7:]:
            os.remove(os.path.join(pasta, arquivo_antigo))
...
# Chaves dos filtros do dashboard — inicializadas aqui para estarem
# disponíveis antes de qualquer rerun
...
# Alinha o botão do popover com os demais via CSS
st.markdown(
    """<style>
...
if st.session_state.pagina == "Dashboard":
    ...
    if df.empty:
        ...
        # --- Flag de reset: aplicado ANTES de instanciar os widgets ---
        ...
        # Garante que o valor salvo ainda existe nas opções
        ...
        with f1:
            filtro_resp = st.selectbox(
                "Responsável", responsaveis_opts,
                index=responsaveis_opts.index(st.session_state.dash_filtro_resp),
                key="dash_filtro_resp",
            )

        with f2:
            filtro_status = st.selectbox(
                "Status", status_opts,
                index=status_opts.index(st.session_state.dash_filtro_status),
                key="dash_filtro_status",
            )

        with f3:
            filtro_crit = st.selectbox(
                "Criticidade", crit_opts,
                index=crit_opts.index(st.session_state.dash_filtro_crit),
                key="dash_filtro_crit",
            )

        with f4:
            busca = st.text_input("🔍 Buscar", placeholder="Macro ação, micro ação...",
                                  key="dash_busca")

        with f5:
            st.markdown(
                '<p style="font-size:0.875rem;margin-bottom:4px;visibility:hidden;">_</p>',
                unsafe_allow_html=True,
            )
        ...
        if filtro_crit   != "TODOS":
            df_filtrado = df_filtrado[df_filtrado["criticidade"]  == filtro_crit]
        if busca.strip():
            _termo = busca.strip().lower()
            df_filtrado = df_filtrado[
                df_filtrado["macro_acao"].fillna("").astype(str).str.lower().str.contains(_termo) |
                df_filtrado["micro_acao"].fillna("").astype(str).str.lower().str.contains(_termo)
            ]
        ...
        st.markdown(
            ...
            <div style="display:flex;gap:0.6rem;margin-bottom:0.5rem;">
                <div style="flex:1;background:#161b27;border:1px solid #1f2535;border-radius:6px;padding:8px 12px;text-align:center;">
                    <div style="font-size:0.65rem;color:#5e6484;text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px;">Total</div>
                    <div style="font-size:1.4rem;font-weight:700;color:#dde1f0;">{total}</div>
                </div>
                <div style="flex:1;background:#161b27;border:1px solid #1f2535;border-radius:6px;padding:8px 12px;text-align:center;">
                    <div style="font-size:0.65rem;color:#5e6484;text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px;">Em Andamento</div>
                    <div style="font-size:1.4rem;font-weight:700;color:#4c7cf3;">{em_andamento}</div>
                </div>
                <div style="flex:1;background:#161b27;border:1px solid #1f2535;border-radius:6px;padding:8px 12px;text-align:center;">
                    <div style="font-size:0.65rem;color:#5e6484;text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px;">Atrasadas</div>
                    <div style="font-size:1.4rem;font-weight:700;color:#{'c94f4f' if atrasadas > 0 else '#dde1f0'};">{atrasadas}</div>
                </div>
                <div style="flex:1;background:#161b27;border:1px solid #1f2535;border-radius:6px;padding:8px 12px;text-align:center;">
                    <div style="font-size:0.65rem;color:#5e6484;text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px;">Concluídas</div>
                    <div style="font-size:1.4rem;font-weight:700;color:#3ecf8e;">{concluidas}</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        # Cálculos para o diagnóstico (usado após o heatmap)
        ...
        for _rp in df_todas["responsavel"].dropna().unique():
            ...
            _sem_rp   = _h_ab / CAPACIDADE_REF  # semanas necessárias para esta pessoa
            _at_rp    = int(_aberto_rp["atrasada"].sum())
            _cr_rp    = int(_aberto_rp["criticidade"].fillna("").astype(str).str.upper().isin(["ALTA"]).sum())
            # Índice normalizado: semanas necessárias vs 1 semana disponível
        ...
        # Monta os 4 intervalos semanais
        _semanas = []
        for _w in range(4):
        ...
        # Para cada pessoa, calcula o índice em cada semana
        _resp_list = sorted(
            [r for r in df_todas["responsavel"].dropna().unique() if str(r).strip()],
        ...
        _html_heat = (
            ...
            '<thead><tr style="background:#111520;">'
        ...
        for _resp in _resp_list:
            ...
            # Para cada tarefa, distribui as horas nas semanas até o prazo
            ...
            for _, _t in _grp_aberto.iterrows():
                ...
                # Descobre em quais semanas esta tarefa precisa ser trabalhada
                _semanas_da_tarefa = []
                for _wi, (_ini, _fim) in enumerate(_semanas):
                    if pd.isna(_pdt) or _pdt > _fim:
                        # Sem prazo ou prazo além das 4 semanas → entra em todas
                        _semanas_da_tarefa.append(_wi)
                    elif _pdt >= _ini:
                        # Prazo dentro ou antes desta semana → entra nesta e nas anteriores
                        _semanas_da_tarefa.append(_wi)
                    # Se o prazo já passou antes desta semana, não entra mais

                if not _semanas_da_tarefa:
                    _semanas_da_tarefa = [0]  # tarefas vencidas vão para semana 1

                _h_por_sem = _h / len(_semanas_da_tarefa)
                for _wi in _semanas_da_tarefa:
                    _horas_por_semana[_wi]   += _h_por_sem
            ...
            _html_heat += (
                ...
                f'font-size:0.78rem;color:#dde1f0;font-weight:600;text-transform:uppercase;">'
                f'{_resp}</td>'
            )

            for _wi in range(4):
                _idx = min(
                    (_horas_por_semana[_wi] / CAPACIDADE_REF) * 100
                    + _atraso_por_semana[_wi]  * 15
                    + _critica_por_semana[_wi] * 8,
                    200
                )
                _cor = _cor_idx(_idx)
                ...
                _html_heat += (
                    ...
                    f'<span style="font-size:0.65rem;color:{_cor};font-weight:700;">{_idx:.0f}%</span>'
                    f'</div>'
                    f'<div style="background:#1f2535;border-radius:99px;height:4px;">'
                    f'<div style="width:{_pct:.0f}%;background:{_cor};height:4px;border-radius:99px;"></div>'
                    f'</div>'
        ...
        # ── Diagnóstico da equipe (após heatmap) ──────────────
        ...
        _sem_prazo       = int(_df_aberto["prazo_dt"].isna().sum())
        _score_medio     = float(_df_aberto["score"].mean()) if not _df_aberto.empty else 0
        _pct_concl_media = float(_df_aberto["percentual_conclusao"].mean()) if not _df_aberto.empty else 0
        _pend_aprov      = int(
            (df_todas["aprovacao_gestor"].fillna("").astype(str).str.upper() == "PENDENTE APROVAÇÃO").sum() +
            (df_todas["aprovacao_conclusao"].fillna("").astype(str).str.upper() == "AGUARDANDO APROVAÇÃO").sum()
        )

        # Pessoa mais sobrecarregada
        _max_idx_val  = 0.0
        _max_idx_resp = "—"
        for _rp in df_todas["responsavel"].dropna().unique():
            _grp_rp = df_todas[df_todas["responsavel"] == _rp]
            _h_ab   = float(_grp_rp[~_grp_rp["status"].fillna("").astype(str).str.upper().isin(STATUS_CONCLUIDOS)]["horas_em_aberto"].sum())
            _at_rp  = int(_grp_rp["atrasada"].sum())
            _cr_rp  = int(_grp_rp["criticidade"].fillna("").astype(str).str.upper().isin(["ALTA"]).sum())
            _idx_rp = (_h_ab / CAPACIDADE_REF) * 100 + _at_rp * 15 + _cr_rp * 8
            if _idx_rp > _max_idx_val:
                _max_idx_val  = _idx_rp
                _max_idx_resp = str(_rp)
        ...
        def _met(titulo, valor, sub, cor="#dde1f0"):
            return (
                ...
                f'<div style="font-size:0.6rem;color:#5e6484;">{sub}</div>'
                f'</div>'
            )
        ...
        _cor_score    = "#c94f4f" if _score_medio > 20 else "#e0a040" if _score_medio > 12 else "#3ecf8e"
        ...
        carga = (
            ...
            .agg(**{
                "QUANTIDADE DE ATIVIDADES": ("id",              "count"),
                "HORAS EM ABERTO":          ("horas_em_aberto", "sum"),
            })
            .reset_index()
        ...
        # Renderiza como HTML para garantir centralização e visibilidade
        def carga_para_html(df: pd.DataFrame) -> str:
            ...
            for _, row in df.iterrows():
                ...
                linhas += f"""
                <tr>
                    <td style="padding:5px 12px;border-bottom:1px solid #1f2535;text-align:center;color:#dde1f0;">{resp}</td>
                    <td style="padding:5px 12px;border-bottom:1px solid #1f2535;text-align:center;font-weight:600;color:#dde1f0;">{qtd}</td>
                    <td style="padding:5px 12px;border-bottom:1px solid #1f2535;text-align:center;font-weight:600;color:#dde1f0;">{hrs}</td>
                </tr>"""
            return f"""
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                <thead>
                    <tr style="background:#111520;">
                        <th style="padding:7px 12px;text-align:center;border-bottom:1px solid #1f2535;color:#5e6484;font-weight:500;text-transform:uppercase;letter-spacing:.07em;">Responsável</th>
                        <th style="padding:7px 12px;text-align:center;border-bottom:1px solid #1f2535;color:#5e6484;font-weight:500;text-transform:uppercase;letter-spacing:.07em;">Quantidade de Atividades</th>
                        <th style="padding:7px 12px;text-align:center;border-bottom:1px solid #1f2535;color:#5e6484;font-weight:500;text-transform:uppercase;letter-spacing:.07em;">Horas em Aberto</th>
                    </tr>
                </thead>
                <tbody>{linhas}</tbody>
            </table>"""
        ...
        st.markdown(
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:-0.5rem;">
                ...
                <a href="data:{_mime};base64,{_excel_b64}"
                   download="{_nome_arquivo}"
                   style="color:#5e6484;font-size:0.75rem;text-decoration:none;white-space:nowrap;">
                   📤 Exportar para Excel
                </a>
            </div>
            """,
        ...
        colunas_exibir = [
            "id", "responsavel", "tipo", "macro_acao", "micro_acao",
            "criticidade", "frequencia", "tempo",
            "prazo", "status", "percentual_conclusao", "score", "atrasada",
        ]
        ...
        tabela_exibir = (
            ...
            .rename(columns={
                ...
                "tipo":                 "Tipo",
                "macro_acao":           "MACRO_AÇÃO",
                "micro_acao":           "MICRO_AÇÃO",
                "criticidade":          "Criticidade",
                "frequencia":           "Frequência",
                "tempo":                "TEMPO",
                "prazo":                "Prazo",
                "status":               "Status",
                "percentual_conclusao": "%_CONCLUSÃO",
                "score":                "SCORE",
                "atrasada":             "ATRASADA",
            })
        )
        ...
        def badge(texto: str, cor: str) -> str:
            return (f'<span style="background:{cor};color:#fff;font-size:0.62rem;'
                    f'padding:2px 8px;border-radius:3px;font-weight:600;white-space:nowrap;">{texto}</span>')
        ...
        COR_STATUS = {"NÃO INICIADA": "#5e6484", "EM ANDAMENTO": "#4c7cf3",
                      "CONCLUÍDA": "#3ecf8e", "PENDENTE APROVAÇÃO": "#e0a040", "PENDENTE": "#e0a040"}
        COR_CRIT   = {"BAIXA": "#3ecf8e", "MÉDIA": "#e0a040", "ALTA": "#e07040"}

        cabecalhos = ["ID", "Tipo", "Macro ação", "Micro ação",
                      "Criticidade", "FREQ.", "TEMPO", "Prazo", "Status", "% CONCLUSÃO", "SCORE"]

        ths = "".join(
            f'<th style="padding:7px 10px;text-align:center;border-bottom:1px solid #1f2535;'
            f'color:#5e6484;font-size:0.68rem;font-weight:500;text-transform:uppercase;'
        ...
        def cel(val, align="center", color="#dde1f0"):
            ...
            return (f'<td style="padding:5px 10px;border-bottom:1px solid #1a1f2e;'
                    f'text-align:{align};color:{color};font-size:0.75rem;">{v}</td>')

        responsaveis_ordenados = sorted(
            tabela_exibir["Responsável"].fillna("—").unique().tolist(),
            key=lambda x: x.lower()
        )
        ...
        for resp in responsaveis_ordenados:
            ...
            html_completo += (
                ...
                f'<span style="font-size:0.95rem;font-weight:600;color:#dde1f0;text-transform:uppercase;'
                f'letter-spacing:.07em;">👤 {resp}</span>'
                f'<span style="background:#1f2535;color:#dde1f0;font-size:0.75rem;padding:1px 8px;'
                f'border-radius:99px;">{total_resp} atividade{"s" if total_resp != 1 else ""}</span>'
                + (f'<span style="background:#c94f4f;color:#fff;font-size:0.75rem;padding:1px 8px;'
                   f'border-radius:99px;">{atrasadas_resp} atrasada{"s" if atrasadas_resp != 1 else ""}</span>'
                   if atrasadas_resp > 0 else "")
                + '</div>'
                f'<div style="margin-bottom:0.5rem;">'
                f'<table style="width:100%;border-collapse:collapse;">'
                f'<thead><tr style="background:#111520;">{ths}</tr></thead><tbody>'
            )
            ...
            for _, row in grupo.iterrows():
                ...
                html_completo += (
                    ...
                    + f'<td style="padding:5px 10px;border-bottom:1px solid #1a1f2e;text-align:center;">'
                      f'{badge(crit, COR_CRIT.get(crit, "#5e6484"))}</td>'
                    + cel(row["Frequência"])
                    + cel(f'{float(row["TEMPO"]):.1f}h')
                    + cel(row["Prazo"], color=prazo_cor)
                    + f'<td style="padding:5px 10px;border-bottom:1px solid #1a1f2e;text-align:center;">'
                      f'{badge(status, COR_STATUS.get(status, "#5e6484"))}</td>'
                    + f'<td style="padding:5px 10px;border-bottom:1px solid #1a1f2e;min-width:90px;">'
                      f'{barra(pct)}</td>'
...
# =========================================================
    ...
    def secao_aprovacao(pendentes, titulo, chave_aprovar, fn_aprovar, chave_reprovar, fn_reprovar, extra_campo=None):
        ...
        if pendentes.empty:
            st.markdown(
                '<div style="font-size:1rem;color:#dde1f0;padding:8px 0;">Nenhuma atividade pendente.</div>',
                unsafe_allow_html=True,
            )
            return
            ...
            with st.expander(f"ID {row['id']}  ·  {row['micro_acao']}"):
                st.markdown(
                    f"""<div style="font-size:1rem;line-height:2;margin-bottom:0.5rem;">
                        ...
                        <br>
                        <span style="color:#5e6484;">Macro ação:</span> {row['macro_acao']}
    ...
    if df.empty:
        ...
        secao_aprovacao(
            pendentes_inicio, "Aprovação de Início",
            "aprovar_inicio", aprovar_inicio,
            "reprovar_inicio", reprovar_inicio,
        )

        st.divider()

        secao_aprovacao(
            pendentes_conclusao, "Aprovação de Conclusão",
            "aprovar_conclusao", aprovar_conclusao,
            "reprovar_conclusao", reprovar_conclusao,
            extra_campo=True,
        )
...
# =========================================================
    ...
    for resp in responsaveis:
        if str(resp).strip() == "":
            ...
            mapa_responsaveis[rotulo] = ""
            opcoes_responsaveis.append(rotulo)
        else:
    ...
    if not opcoes_responsaveis:
        ...
        df_filtrado = df[
            df["responsavel"].fillna("").astype(str).str.strip() == responsavel_filtro
        ].copy()
        ...
        if df_filtrado.empty:
            ...
            for _, row in df_filtrado.iterrows():
                _cor_badge = {
                    ...
                    "EM ANDAMENTO":       "#4c7cf3",
                    "CONCLUÍDA":          "#3ecf8e",
                    "PENDENTE APROVAÇÃO": "#e0a040",
                    "PENDENTE":           "#e0a040",
                }.get(str(row["status"]).strip(), "#5e6484")
                ...
                with _col_badge:
                    st.markdown(
                        ...
                        f'height:2.1rem;margin-top:0.1rem;">'
                        f'<span style="background:{_cor_badge};color:#fff;font-size:0.65rem;'
                        f'padding:4px 0;border-radius:4px;font-weight:600;letter-spacing:0.05em;'
                        f'text-align:center;display:block;width:100%;">{row["status"]}</span></div>',
                        unsafe_allow_html=True,
                    )
                ...
                with _col_exp:
                    with st.expander(f"ID {row['id']} | {row['micro_acao']}"):
                        ...
                        if not modo_edicao:
                            st.markdown(
                                f"""
                                <div style="line-height:1.8;margin-bottom:0.4rem;">
                            ...
                            with _e1:
                                ed_tipo = st.selectbox("Tipo", OPCOES_TIPO,
                                    index=OPCOES_TIPO.index(row["tipo"]) if row["tipo"] in OPCOES_TIPO else 0,
                                    key=f"ed_tipo_{row['id']}")
                            ...
                            with _e2:
                                ed_crit = st.selectbox("Criticidade", OPCOES_CRITICIDADE,
                                    index=OPCOES_CRITICIDADE.index(row["criticidade"]) if row["criticidade"] in OPCOES_CRITICIDADE else 0,
                                    key=f"ed_crit_{row['id']}")
                                ed_freq = st.selectbox("Frequência", OPCOES_FREQUENCIA,
                                    index=OPCOES_FREQUENCIA.index(row["frequencia"]) if row["frequencia"] in OPCOES_FREQUENCIA else 0,
                                    key=f"ed_freq_{row['id']}")
                                ed_tempo = st.number_input("Tempo (horas)", min_value=0.0, step=0.5,
                                    value=float(row["tempo"]) if pd.notna(row["tempo"]) else 0.0,
                                    key=f"ed_tempo_{row['id']}")
                            ...
                            with _s1:
                                if st.button("💾 Salvar edição", key=f"salvar_ed_{row['id']}", use_container_width=True):
                                    editar_atividade(int(row["id"]), {
                                        "tipo": ed_tipo, "macro_acao": ed_macro, "micro_acao": ed_micro,
                                        "criticidade": ed_crit, "frequencia": ed_freq,
                                        "tempo": ed_tempo, "prazo": ed_prazo, "responsavel": ed_resp,
                                    })
                        ...
                        percentual_atual = float(row["percentual_conclusao"]) if pd.notna(row["percentual_conclusao"]) else 0.0
                        ...
                        if not modo_edicao:
                            ...
                            novo_status = st.selectbox(
                                f"Novo status — atividade {row['id']}",
                                OPCOES_STATUS,
                            ...
                            if novo_status == "CONCLUÍDA":
                                st.session_state[chave_perc] = 100

                            percentual = st.slider(
                                f"% de conclusão — atividade {row['id']}",
                                0, 100,
                                key=chave_perc,
                            )
                                ...
                                if not st.session_state.get(chave_confirmar, False):
                                    ...
                                    with cc1:
                                        if st.button("Sim, apagar", key=f"confirm_sim_{row['id']}"):
                                            ...
                                            st.success(f"Atividade {row['id']} apagada com sucesso.")
                                            st.rerun()
                                    with cc2:
                        ...
                        if not hist.empty:
                            st.divider()
                            st.markdown(
                                '<div style="font-size:0.72rem;font-weight:600;color:#5e6484;'
                                'text-transform:uppercase;letter-spacing:.07em;margin-bottom:4px;">Histórico</div>',
                                unsafe_allow_html=True,
                            )
                            for _, h in hist.iterrows():
                                st.markdown(
                                    ...
                                    + (f' · <span style="color:#dde1f0;">{h["campo"]}</span>: '
                                       ...
                                       f'<span style="color:#3ecf8e;">{h["valor_novo"]}</span>'
                                       if h["campo"] else "")
                                    + '</div>',
                                    unsafe_allow_html=True,
                                )
...
# =========================================================
    ...
    # Mensagem de sucesso em largura total — aparece no topo antes do formulário
    ...
    # Estado do fluxo: False = formulário, True = confirmação
    ...
    if not st.session_state.nova_pendente:
        ...
        if st.button("Salvar atividade", type="primary"):
            ...
            # Guarda tudo no session_state para usar na etapa de confirmação
        ...
        # ── ETAPA 2: Painel de impacto + confirmação ──────────
        d = st.session_state.nova_dados

        # Resolve responsável e prazo
        if d["escolha"] == "Novo responsável":
            responsavel_final = d["nome_novo"].strip()
        elif d["escolha"] == "(Sem responsável)":
            responsavel_final = ""
        else:
            responsavel_final = d["escolha"]
        ...
        # Calcula impacto se responsável existir na base
        _tem_impacto = (
            responsavel_final and
            not _df_impacto.empty and
        ...
        if _tem_impacto:
            ...
            _dias_ate_iniciar = _horas_antes / 8.0
            _data_inicio_est  = _hoje_nova + pd.Timedelta(days=_dias_ate_iniciar)
            ...
            # Período coberto pela fila atual (hoje → prazo mais distante)
            ...
            # Nota sobre atividade recorrente
            _mapa_freq_txt = {
            ...
            _html = (
                ...
                '<div style="font-size:0.65rem;font-weight:700;color:#5e6484;text-transform:uppercase;letter-spacing:.1em;margin-bottom:12px;display:flex;justify-content:space-between;">'
                '<span>Previs&atilde;o de Execu&ccedil;&atilde;o</span>'
                f'<span style="color:{"#c94f4f" if _vai_atrasar else "#3ecf8e"};font-weight:700;">{"&#9888; Risco de atraso" if _vai_atrasar else "&#10003; Dentro do prazo"}</span>'
                '</div>'
                ...
                f'<div style="font-size:0.6rem;color:#5e6484;margin-top:2px;">{_periodo_fila}</div>'
                f'</div>'
                ...
                f'<div style="font-size:0.6rem;color:#5e6484;">{_horas_antes:.0f}h de backlog antes desta</div>'
                f'</div>'
                ...
                f'<div style="font-size:0.6rem;color:#5e6484;">estimativa baseada na fila atual</div>'
                f'</div>'
                ...
                f'<div style="font-size:0.6rem;color:{"#c94f4f" if _vai_atrasar else "#5e6484"};">{"&#9888; entrega prevista ap&oacute;s o prazo" if _vai_atrasar else "data limite definida"}</div>'
                f'</div>'
                ...
                f'<div>'
                f'<div style="font-size:0.58rem;color:#5e6484;text-transform:uppercase;margin-bottom:2px;">Tempo para execu&ccedil;&atilde;o</div>'
                f'<div style="font-size:0.82rem;font-weight:600;color:#dde1f0;">{int(_h_nv)}h estimadas &asymp; {max(1,round(_h_nv/8))} dia{"s" if round(_h_nv/8) != 1 else ""} &uacute;til{"" if round(_h_nv/8) != 1 else ""}</div>'
                f'</div>'

                f'<div>'
                f'<div style="font-size:0.58rem;color:#5e6484;text-transform:uppercase;margin-bottom:2px;">Margem at&eacute; o prazo</div>'
                + (
                    f'<div style="font-size:0.82rem;font-weight:600;color:#c94f4f;">&#8722;{abs(int((_prazo_ts - _data_concl_est).days))} dia(s) de atraso</div>'
                    if _vai_atrasar else
                    f'<div style="font-size:0.82rem;font-weight:600;color:#3ecf8e;">+{int((_prazo_ts - _data_concl_est).days)} dia(s) de folga</div>'
                ) +
                f'</div>'

                f'<div>'
                f'<div style="font-size:0.58rem;color:#5e6484;text-transform:uppercase;margin-bottom:2px;">Score desta atividade</div>'
                f'<div style="font-size:0.82rem;font-weight:600;color:#4c7cf3;">{_score_nv:.0f} pontos &middot; {d["criticidade"]} &middot; {d["frequencia"]}</div>'
                f'</div>'
                ...
                + (
                    ...
                    f'<div style="font-size:0.7rem;color:#5e6484;margin-top:3px;">'
                    f'Carga recorrente estimada: <b style="color:#dde1f0;">~{_horas_mes:.0f}h/m&ecirc;s</b> para este respons&aacute;vel. '
                    f'A an&aacute;lise acima refere-se apenas ao <b>primeiro ciclo</b>; nas pr&oacute;ximas recorr&ecirc;ncias, a posi&ccedil;&atilde;o na fila ser&aacute; recalculada com base no score vigente.'
                    f'</div></div>'
                ...
                'Sistema de Gest&atilde;o de Atividades &middot; An&aacute;lise gerada automaticamente'
                '</div>'
                '</div>'
            )
            ...
            # Responsável novo ou sem histórico: mostra resumo simples
...
# =========================================================
# DEFINIÇÕES E MEMÓRIA DE CÁLCULO
# =========================================================
elif st.session_state.pagina == "Simulador":

    st.markdown("### 🔮 Simulador de Demanda")
    st.markdown(
        '<div style="font-size:0.82rem;color:#5e6484;margin-bottom:1rem;">'
        'Preencha os dados da demanda hipotética e veja instantaneamente o impacto na equipe.'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    if df_sim.empty:
        ...
        responsaveis_sim = sorted(
            [r for r in df_sim["responsavel"].dropna().unique() if str(r).strip()],
            key=lambda x: x.lower()
        )
        ...
        if st.session_state.get("sim_btn"):
            ...
            _html_sim = (
                ...
                '<div style="font-size:0.65rem;font-weight:700;color:#5e6484;text-transform:uppercase;letter-spacing:.1em;margin-bottom:12px;display:flex;justify-content:space-between;">'
                '<span>Previs&atilde;o de Execu&ccedil;&atilde;o</span>'
                f'<span style="color:{"#c94f4f" if _vai_atr_sim else "#3ecf8e"};font-weight:700;">{"&#9888; Risco de atraso" if _vai_atr_sim else "&#10003; Dentro do prazo"}</span>'
                '</div>'
                ...
                f'<div style="font-size:0.6rem;color:#5e6484;margin-top:2px;">Per&iacute;odo da fila: {_periodo_sim}</div>'
                f'</div>'

                f'<div style="background:#161b27;border-radius:6px;padding:10px 12px;border-left:3px solid #5e6484;">'
                f'<div style="font-size:0.58rem;color:#5e6484;text-transform:uppercase;margin-bottom:3px;">In&iacute;cio estimado</div>'
                f'<div style="font-size:1.1rem;font-weight:700;color:#dde1f0;">{_inicio_est_sim.strftime("%d/%m/%Y")}</div>'
                f'<div style="font-size:0.6rem;color:#5e6484;">{_horas_antes_sim:.0f}h de backlog antes desta</div>'
                f'</div>'
                ...
                f'<div style="font-size:0.6rem;color:#5e6484;">Prazo definido: <b style="color:#dde1f0;">{_prazo_sim.strftime("%d/%m/%Y")}</b></div>'
                f'</div>'
                ...
                f'<div>'
                f'<div style="font-size:0.58rem;color:#5e6484;text-transform:uppercase;margin-bottom:2px;">Tempo para execu&ccedil;&atilde;o</div>'
                f'<div style="font-size:0.82rem;font-weight:600;color:#dde1f0;">{int(_h_nova)}h &asymp; {max(1,round(_h_nova/8))} dia{"s" if round(_h_nova/8) != 1 else ""} &uacute;til{"" if round(_h_nova/8) <= 1 else "is"}</div>'
                f'</div>'

                f'<div>'
                f'<div style="font-size:0.58rem;color:#5e6484;text-transform:uppercase;margin-bottom:2px;">Margem at&eacute; o prazo</div>'
                + (
                    f'<div style="font-size:0.82rem;font-weight:600;color:#c94f4f;">&#8722;{abs(_margem_sim)} dia(s) de atraso</div>'
                ...
                f'<div>'
                f'<div style="font-size:0.58rem;color:#5e6484;text-transform:uppercase;margin-bottom:2px;">Score desta atividade</div>'
                f'<div style="font-size:0.82rem;font-weight:600;color:#4c7cf3;">{_score_nova:.0f} pontos &middot; {sim_crit} &middot; {sim_freq}</div>'
                f'</div>'
                ...
                + (
                    ...
                    f'<div style="font-size:0.7rem;color:#5e6484;margin-top:3px;">'
                    f'Carga recorrente estimada: <b style="color:#dde1f0;">~{_horas_mes_sim:.0f}h/m&ecirc;s</b>. '
                    f'A an&aacute;lise refere-se apenas ao <b>primeiro ciclo</b>; nas pr&oacute;ximas recorr&ecirc;ncias a posi&ccedil;&atilde;o ser&aacute; recalculada.'
                    f'</div></div>'
            ...
            # ── Fila única estilo campeonato ──────────────────────
            st.divider()
            st.markdown(
                '<div style="font-size:0.72rem;font-weight:600;color:#5e6484;text-transform:uppercase;'
                'letter-spacing:.08em;margin-bottom:4px;">Fila de prioridade</div>'
            ...
            # Posição original de cada atividade (sem a nova)
            ...
            # Fila com a nova demanda inserida
            _fila = _aberto_resp[["id","micro_acao","macro_acao","criticidade","prazo","status","score","atrasada"]].copy()
            _fila["_nova"] = False

            _label_micro = sim_micro if sim_micro.strip() else "(nova demanda)"
            _label_macro = sim_macro if sim_macro.strip() else "—"

            _nova_row = {
                "id": "—", "micro_acao": _label_micro, "macro_acao": _label_macro,
            ...
            _ths2 = "".join(
                f'<th style="padding:6px 10px;text-align:center;border-bottom:1px solid #1f2535;'
                f'color:#5e6484;font-size:0.65rem;font-weight:500;text-transform:uppercase;white-space:nowrap;">{h}</th>'
            ...
            for _pos, (_, _r) in enumerate(_fila.iterrows(), 1):
                ...
                if _nova_flag:
                    ...
                    _deslocou  = _pos_ant < _pos   # estava antes, foi pra depois = desceu
                    if _deslocou:
                        _seta_html = '<span style="font-size:0.85rem;color:#c94f4f;">▼</span>'
                    else:
                        _seta_html = '<span style="font-size:0.85rem;color:#3ecf8e;">—</span>'
                ...
                _rows2 += (
                    ...
                    f'<td style="padding:5px 10px;border-bottom:1px solid #1a1f2e;text-align:left;font-size:0.75rem;color:#dde1f0;">'
                    f'{_r["micro_acao"]}</td>'
                    f'<td style="padding:5px 8px;border-bottom:1px solid #1a1f2e;text-align:center;">'
                    f'<span style="background:{COR_C2.get(_c,"#5e6484")};color:#fff;font-size:0.58rem;padding:1px 6px;border-radius:3px;font-weight:600;">{_c}</span></td>'
                    ...
                    f'<td style="padding:5px 8px;border-bottom:1px solid #1a1f2e;text-align:center;">'
                    f'<span style="background:{COR_S2.get(_s,"#5e6484")};color:#fff;font-size:0.58rem;padding:1px 6px;border-radius:3px;font-weight:600;">{_s}</span></td>'
            ...
            _n_deslocadas = sum(
                1 for _, r in _fila.iterrows()
                if not r["_nova"] and _pos_orig.get(int(float(r["id"])), 0) < list(_fila.index).index(_fila[~_fila["_nova"]].index[list(_fila[~_fila["_nova"]]["id"]).index(r["id"])]) + 1
            ) if not _aberto_resp.empty else 0

            st.markdown(
                f'<table style="width:100%;border-collapse:collapse;">'
                f'<thead><tr style="background:#111520;">{_ths2}</tr></thead>'
                f'<tbody>{_rows2}</tbody></table>',
                unsafe_allow_html=True,
            )
            st.markdown(
    ...
    def bloco(titulo, conteudo, cor_borda="#4c7cf3"):
        st.markdown(
            ...
            f'<div style="font-size:0.85rem;color:#dde1f0;line-height:1.9;">{conteudo}</div>'
            f'</div>',
            unsafe_allow_html=True,
        )
    ...
    def var(nome, descricao):
        return (
            f'<div style="display:flex;gap:0.5rem;margin-top:4px;">'
            f'<span style="color:#4c7cf3;font-weight:600;white-space:nowrap;min-width:220px;">{nome}</span>'
            f'<span style="color:#5e6484;">→</span>'
            f'<span style="color:#dde1f0;">{descricao}</span>'
            f'</div>'
        )

    def formula(expr):
        return (
            f'<div style="background:#111520;border:1px solid #1f2535;border-radius:4px;'
            f'padding:8px 14px;margin:8px 0;font-family:monospace;font-size:0.82rem;color:#3ecf8e;">{expr}</div>'
        )

    def subtitulo(texto):
        st.markdown(
            f'<div style="font-size:0.72rem;font-weight:600;color:#5e6484;text-transform:uppercase;'
            f'letter-spacing:.1em;margin-bottom:0.6rem;margin-top:1.2rem;">{texto}</div>',
            unsafe_allow_html=True,
        )

    st.markdown("### Definições e Memória de Cálculo")
    st.markdown(
        '<div style="font-size:0.82rem;color:#5e6484;margin-bottom:1.5rem;">'
        'Referência completa de todos os conceitos, classificações e fórmulas utilizados no sistema. '
        'Cada cálculo é explicado com suas variáveis para total transparência.'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    bloco("Horas em Aberto", f"""
        ...
        Atividade de 8h com 0% → <b>8h em aberto</b><br>
        Atividade concluída → sempre <b>0h em aberto</b>.
    """)
    ...
    bloco("Score de Prioridade", f"""
        ...
        <br>
        <b>Score máximo fixo:</b> (4×3) + (5×2) + (4×2) + (5×3) = 45, mais o peso de tempo.<br>
        Scores acima de 30 indicam atividades críticas e urgentes.
    """)
    ...
    bloco("Índice de Comprometimento — Heatmap das 4 Semanas", f"""
        ...
        {var("Horas em Aberto", "Trabalho restante da atividade conforme cálculo anterior.")}
        {var("Nº de semanas", "Quantas das 4 semanas ficam dentro do horizonte até o prazo. Mínimo 1.")}
        ...
        {var("Críticas × 8", "Cada atividade Alta ou Crítica em aberto soma 8 pontos percentuais.")}
        <br><b>Classificação:</b><br>
        🟢 Disponível → até 70% &nbsp;|&nbsp;
        🟡 Moderado → 71–90% &nbsp;|&nbsp;
        🟠 Alto → 91–120% &nbsp;|&nbsp;
        🔴 Sobrecarregado → acima de 120%
    """)

    bloco("Diagnóstico da Equipe", f"""
        Painel de saúde geral da equipe com memória de cada indicador:<br><br>
        <b>Horas em Aberto (equipe):</b>
        {formula("Σ Horas em Aberto de todas as atividades com status ≠ Concluída")}
        <b>Déficit de Capacidade:</b>
        {formula("Déficit = Horas em Aberto − (Nº Pessoas × 40h × 4 semanas)")}
        {var("Nº Pessoas", "Total de responsáveis distintos com atividades cadastradas.")}
        {var("40h × 4 = 160h", "Capacidade disponível de cada pessoa nas próximas 4 semanas.")}
        {var("Resultado positivo", "Há trabalho além da capacidade — essas horas só ocorrem após as 4 semanas.")}
        <br><b>Semanas para Zerar Backlog:</b>
        {formula("Semanas = Horas em Aberto ÷ (Nº Pessoas × 40h)")}
        {var("Premissa", "Nenhuma nova demanda criada e todos trabalham em plena capacidade de 40h/semana.")}
        <br><b>Índice Individual (Equipe em Sobrecarga e Mais Sobrecarregada):</b>
        ...
        <br><b>Taxa de Atraso:</b>
        {formula("Taxa (%) = (Atividades vencidas ÷ Total de atividades visíveis) × 100")}
        <b>Críticas em Aberto:</b> atividades com criticidade Alta ou Crítica com status ≠ Concluída.
    """)
    ...
    bloco("Regra de Atividade Atrasada", f"""
        Uma atividade é classificada como atrasada quando ambas as condições são verdadeiras:
        {formula("Atrasada = (Data do Prazo < Data de Hoje) E (Status ≠ Concluída)")}
        {var("Data do Prazo", "Data limite informada manualmente ou calculada automaticamente.")}
        {var("Data de Hoje", "Data atual no momento em que o sistema carrega os dados.")}
    ...
    bloco("Histórico de Alterações", """
        Registra automaticamente toda ação relevante em uma atividade:<br><br>
        &bull; Atualização de status e percentual de conclusão<br>
        &bull; Aprovação ou reprovação de início e de conclusão pelo gestor<br>
        &bull; Edição de campos — apenas o que foi alterado é registrado, com valor anterior e novo<br><br>
        Cada registro contém: <b>data e hora</b>, <b>tipo de ação</b>, <b>campo alterado</b>,
        <b>valor anterior</b> e <b>valor novo</b>.<br>
        Visível ao expandir qualquer atividade em "Atualizar Atividades".
    """)

    bloco("Backup Automático", """
        A cada inicialização do sistema, uma cópia do banco é salva na pasta
        <code style="background:#1f2535;padding:1px 6px;border-radius:3px;">backups/</code>:<br><br>
        &bull; Nome: <code style="background:#1f2535;padding:1px 6px;border-radius:3px;">gestao_atividades_AAAAMMDD.db</code><br>
        &bull; Um backup por dia — se já existe, nenhum novo é criado naquele dia<br>
        &bull; Máximo de <b>7 backups</b> mantidos; os mais antigos são excluídos automaticamente<br><br>
        Para restaurar: copiar o arquivo de <code style="background:#1f2535;padding:1px 6px;border-radius:3px;">backups/</code>
        e renomear para <code style="background:#1f2535;padding:1px 6px;border-radius:3px;">gestao_atividades.db</code>.
    """)

    bloco("Importação via Excel", """
        O sistema lê automaticamente todas as abas da planilha. Colunas reconhecidas:<br><br>
        <code style="background:#1f2535;padding:2px 8px;border-radius:3px;font-size:0.78rem;">
        TIPO | MACRO AÇÃO | MICRO AÇÃO | CRITICIDADE | FREQUÊNCIA | TEMPO | PRAZO | STATUS | RESPONSÁVEL | % CONCLUSÃO
        </code><br><br>
        &bull; Variações com ou sem acento são aceitas (ex: FREQUENCIA = FREQUÊNCIA)<br>
        &bull; Linhas sem micro ação são ignoradas automaticamente<br>
        &bull; O nome da aba fica registrado como origem da atividade<br>
        &bull; Todas as atividades importadas recebem aprovação automática do gestor<br>
        &bull; Recomenda-se limpar a base antes de reimportar para evitar duplicatas
    """)

    bloco("Simulação de Demanda — em breve", """
        Funcionalidade planejada que permitirá simular a inclusão de uma nova atividade antes de aprová-la:<br><br>
        &bull; Verificar se o responsável tem capacidade com base no índice de comprometimento<br>
        &bull; Identificar quais atividades teriam o score reduzido na fila de prioridade<br>
        &bull; Antecipar quais atividades correm risco de atraso caso a nova demanda seja assumida<br><br>
        A simulação não altera dados — é apenas uma prévia do impacto antes da aprovação.
    """, cor_borda="#5e6484")
...
st.markdown(
    ...
    <div style="margin-top:3rem;text-align:center;color:#1f2535;font-size:0.82rem;letter-spacing:0.08em;text-transform:uppercase;">
        Software Livre e de Uso Aberto — Sinta-se à Vontade para Usar, Adaptar e Evoluir.
    </div>
    """,
    unsafe_allow_html=True,
)